...
export type SeatMap = Map<string, Status>;

export const toVec = (str: string) => str.split(":").map(x => parseInt(x, 10));
export const toStr = (vec: number[]) => vec.join(":");

export const init = (data: string[]): SeatMap => {
  const seatMap: SeatMap = new Map();
  for (let i = 0; i < data.length; i++) {
    for (let j = 0; j < data[i].length; j++) {
      seatMap.set(
        toStr([i, j]),
        data[i][j] === "." ? Status.FLOOR : Status.EMPTY
      )
    }
  }
  return seatMap;
...
const actual = `LLLLLLLLLLLLLLLLLLLLLLLLL.LLLLL..LLLLLLLLLLLL.LLL..LLLLLLLLLLLLLL.LL.LLLLL.LLLLLLLLL.LLLLLLLLLLLLL
...
LLLLLLL.LLLLLLL..LLLLLLLLLLLLLLLLLLL.LLLLLLLLLLLLL.LLLLLLLLL.LL.LLLL.LLLLL.LLLLLLLLL.LLLLLLL.LLLLL`.split("\n");
